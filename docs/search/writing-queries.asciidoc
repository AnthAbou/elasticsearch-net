...
this will return the first 10 documents, giving them all a `_score` of 1.0

[NOTE]
--
You can use `.From()` and `.Size()` to paginate results or retrieve more documents from the initial load. Note that `.From()` + `.Size()` can not be more than the `index.max_result_window` index setting, which defaults to 10,000.
